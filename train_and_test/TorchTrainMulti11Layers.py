...
# from model import GeneratorLayers
...
checkpoints_path = result_path + 'checkpoints/fusion_11_layers_secondtime/'
image_log_path = result_path + 'image_log/fusion_11_layers_secondtime/'  # 保存训练过程中的一部分图
# model_save_step = 500
img_log_step = 50
lr = 1e-4 * 0.5  # leaning rate , decrease the learning rate again
batch_size = 1  # batch size
num_workers = 0
shuffle_flag = True
pretrain_flag = True  # 不加载预训练的权重
...
# load model , set 15 layers
netG = Light(33 , 3).cuda()
...
if pretrain_flag:
    netG.load_state_dict(torch.load(checkpoints_path + 'netG_epoch_%d_%d.pth' % (3 , 2357)))
    netD1.load_state_dict(torch.load(checkpoints_path + 'netD1_epoch_%d_%d.pth' % (3 , 2357)))
# set optimizer
optimizerD1 = optim.Adam(netD1.parameters(), lr = lr * 0.5) #将判别器的学习率设置为生成器的0.5倍
...
if __name__ == "__main__":
    ...
    for epoch in range(3, stage_epoch):
        for i, (img20x_data , img20x_label) in enumerate(train_loader):

            if epoch == 3 and i < (2357 - 2235) * img_log_step:
                continue
            ...
            if i % img_log_step == 0:
                ...
                cv2.imwrite(image_log_path + 'stage1_' + str(epoch) + '_' + str(image_save_counter) + '_i.tif', img20x_data)
                cv2.imwrite(image_log_path + 'stage1_' + str(epoch) + '_' + str(image_save_counter) + '_l.tif', label_20x)
