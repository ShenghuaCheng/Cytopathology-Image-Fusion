...
#set blur seg model
model_path = '/mnt/zhaolu_g/GXB/20x_and_40x_data/blur_task/train_data/train_second/middle_result/weights_log/44_1125.pth'
...
pretrain_flag = False
if pretrain_flag:
    netG.load_state_dict(torch.load(checkpoints_path + 'netG_epoch_%d_%d.pth' % (2 , 768)))
    netD1.load_state_dict(torch.load(checkpoints_path + 'netD1_epoch_%d_%d.pth' % (2 , 768)))
...
if __name__ == "__main__":
    ...
    for epoch in range(1 , NUM_EPOCHS + 1):
        ...
        for i, (img20x_data , img20x_data_c , img20x_label) in enumerate(train_loader):
            ...
            # 总loss
            adver_loss = 0.001 * adver_loss1
            g_loss = image_mse1 + adver_loss
            ...
            print('%s '
                  '[epoch %d/%d] [batch %d/%d]'
                  '[D loss: %.4f, r : %.4f, f : %.4f]'
                  '[G loss: %.4f, adv : %.4f, mse : %.4f]' % ( cur_time ,
            epoch, NUM_EPOCHS, i, len(train_loader), d_loss.item(), real_loss.item() , fake_loss.item() ,
             g_loss.item() , adver_loss.item() , image_mse1.item()))
