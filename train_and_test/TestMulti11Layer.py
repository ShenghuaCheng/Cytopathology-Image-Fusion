...
# run_log_path = 'X:\\GXB\\20x_and_40x_data\\test_result\\fusion_11_layers_secondtime\\'
...
def test_model_mean():
    ...
    model_path = 'X:\\GXB\\20x_and_40x_data\\checkpoints\\fusion_11_layers_secondtime\\'
    result_path = 'X:\\GXB\\20x_and_40x_data\\test_result\\fusion_11_layers_secondtime_multi_models\\'
    test_list = ['netG_epoch_3_2517.pth', 'netG_epoch_3_2516.pth', 'netG_epoch_3_2512.pth', 'netG_epoch_3_2509.pth',
                 'netG_epoch_3_2503.pth']

    log_writer = SummaryWriter(result_path + 'fusion_11_layers_multi_models.log')
    log_file = open(result_path + 'fusion_11_layers_multi_models.txt' , 'w')
...
def test_model():
    ...
    model_path = 'X:\\GXB\\20x_and_40x_data\\checkpoints\\fusion_11_layers_secondtime\\'
    result_path = 'X:\\GXB\\20x_and_40x_data\\test_result\\fusion_11_layers_secondtime_multi_imgs\\'
    test_list = ['netG_epoch_3_2517.pth']
        #, 'netG_epoch_2_2159.pth' , 'netG_epoch_2_2160.pth' , 'netG_epoch_2_2161.pth' , 'netG_epoch_2_2162.pth']

    log_writer = SummaryWriter(result_path + 'fusion_11_layers_multi_imgs.log')
    log_file = open(result_path + 'fusion_11_layers_multi_imgs.txt' , 'w')
    ...
    for weight in test_list:
        ...
        for data , label in zip(datas , labels):
            ...
            with torch.no_grad():
                gen_label = model_g(data)
                en_avg, sd_avg, ssim_avg, cc_avg, sf_avg, vif_avg = save_result_and_evaluate(layers, result_path,
                                                                                             weight, data[0],
                                                                                             gen_label[0], label[0], k)

            log_writer.add_scalars('scalar/en', {'en_0': en_avg[0], 'en_g': en_avg[1], 'en_f': en_avg[2]}, k)
            log_file.write(
                str(k) + '-' + 'en_0:' + str(en_avg[0]) + 'en_g:' + str(en_avg[1]) + 'en_f:' + str(en_avg[2]) + '\n')
            log_writer.add_scalars('scalar/sd', {'sd_0': sd_avg[0], 'sd_g': sd_avg[1], 'sd_f': sd_avg[2]}, k)
            log_file.write(
                str(k) + '-' + 'sd_0:' + str(sd_avg[0]) + 'sd_g:' + str(sd_avg[1]) + 'sd_f:' + str(sd_avg[2]) + '\n')
            ...
            log_file.write(
                str(k) + '-' + 'sf_0:' + str(sf_avg[0]) + 'sf_g:' + str(sf_avg[1]) + 'sf_f:' + str(sf_avg[2]) + '\n')
...
def save_result_and_evaluate(layers , result_path , weight , data , gen_label , label , img_k):
    ...
    gen20x_label = np.transpose(gen20x_label, [1, 2, 0]) * 255
    img20x_label = np.transpose(img20x_label, [1, 2, 0]) * 255
    img20x_data = cv2.hconcat(np.uint8(temp))
    label_20x = cv2.hconcat((np.uint8(gen20x_label), np.uint8(img20x_label)))

    img20x_data = cv2.cvtColor(img20x_data, cv2.COLOR_RGB2BGR)
    label_20x = cv2.cvtColor(label_20x, cv2.COLOR_RGB2BGR)

    cv2.imwrite(result_path + weight + '\\'  + str(img_k) + '_i.tif', img20x_data)
    cv2.imwrite(result_path + weight + '\\'  + str(img_k) + '_l.tif', label_20x)

    layer_0 = int(len(layers) / 2)
    img20x_layer_0 = img20x_data[: , layer_0 * 512 : (layer_0 + 1) * 512 , :]

    gen20x_label = label_20x[: , 0 : 512 , :]
    img20x_label = label_20x[:, 512: 1024, :]
