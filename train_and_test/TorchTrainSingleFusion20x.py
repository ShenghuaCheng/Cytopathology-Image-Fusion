...
checkpoints_path = path + 'checkpoints/fusion_0_layer/'
image_log_path = path + 'image_log/fusion_0_layer/'
tensorboard_log = path + 'run_log/fusion_0_layer/'
# model_save_step = 500
img_log_step = 50
...
# set optimizer
optimizerD1 = optim.Adam(netD1.parameters(), lr = lr * 0.5) #decrease the discriminator's lr
...
if __name__ == "__main__":
    ...
    #在原有5轮的基础上再次训练3轮，设置保持不变
    for epoch in range(6 , stage_epoch + 4):
        for i, (img20x_data , img20x_label) in enumerate(train_loader):
            if epoch == 6 and i < (4281 - 3724) * img_log_step:
                continue
