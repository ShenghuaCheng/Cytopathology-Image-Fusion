...
result_path = 'X:\\GXB\\20x_and_40x_data\\other_data_test\\test_our_data\\'
...
def save_img(ors , cz , position , block_size , level = 0):
    ...
    thre_vol = (62.5 * 4 ** 3) * 0.2431 / 0.293
...
def generate_sample(path , cz):
    ...
    h_min = img_bin.nonzero()[0][0] * 4 ** bin_level + 1000
    h_max = img_bin.nonzero()[0][-1] * 4 ** bin_level - 1000
    w_min = 1000
    w_max = ors.level_dimensions[0][0] - 1000

    block_size = int(512 * 0.2431 / 0.293)
    overlab = int(block_size / 4)
...
if __name__ == '__main__':
    mrxses_path = 'X:\\TCT\\TCTDATA\\our\\OriginalData\\'
    czs = os.listdir(mrxses_path)
    czs = [x for x in czs if x.find('.svs') != -1]
