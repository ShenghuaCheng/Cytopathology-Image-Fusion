...
from compare_fusion.data_loader import Pixel2PixelDataLoader
...
checkpoints_path = path + 'checkpoints/pixel2pixel_0_layer/'
image_log_path = path + 'image_log/pixel2pixel_0_layer/'
tensorboard_log = path + 'run_log/pixel2pixel_0_layer/'
# model_save_step = 500
img_log_step = 50
...
layers = [0]
train_set = Pixel2PixelDataLoader(image_20x_data_path , image_20x_label_path , names_log , layers)
train_loader = DataLoader(dataset=train_set, batch_size=batch_size, shuffle=shuffle_flag , num_workers=num_workers)

# load model
netG = Pixel2PixelGenerator(3 , 3).cuda()
print('# generator parameters:', sum(param.numel() for param in netG.parameters()))
netD = Pixel2PixleDiscriminator(3 , 3).cuda()
...
pretrain_flag = False
if pretrain_flag:
    netG.load_state_dict(torch.load(checkpoints_path + 'netG_epoch_%d_%d.pth' % (1 , 178)))
    netD.load_state_dict(torch.load(checkpoints_path + 'netD1_epoch_%d_%d.pth' % (1 , 178)))
...
if __name__ == "__main__":
    ...
    optimizerD = optim.Adam(netD.parameters(), lr=lr * 0.125, betas=(0.9, 0.999))
